...
coef_names <- names(coefficients(logistic_counts))[-1]

x_labels <- c("C>G", "C>T", "CpG>TpG", "T>C", "T>G", "C>A", "T>A")

plts <- lapply(1:length(coef_names), function(i) {
  p <- plot(ggpredict(logistic_counts, coef_names[i]))
  p + theme(plot.title = element_text(hjust = 0.5, size = 14)) +
    xlab(x_labels[i]) + ylab("Probability") +
    ggtitle(NULL)
})

p <- wrap_plots(plts)

ggsave("figure_07_log_regr.png", plot = p, width = 8, height = 5, path = "./../../Figures/")
```
